...
When you finish generating the workflow:

1) Create/commit exactly one file: `.github/workflows/fast-pr-gate.yml` with the final YAML contents.

2) **Mandatory:** Provide a clear, detailed explanatory summary in the Pull Request description/body (or as a separate top-level response if no PR body is available). This summary MUST include:
   - What mise tasks were detected (list exact task names found, e.g., lint, format-check).
   - Which check steps were included and why (e.g., "Included mise run lint and mise run format-check because they exist; included uv run pytest as fallback since no mise test task exists").
   - Which potential steps were skipped and why (e.g., "Skipped any type-check step because no mypy/equivalent task detected").
   - Dependency install choice and rationale (e.g., "Used uv sync --frozen because uv.lock exists").
   - Any assumptions made (e.g., "Assumed default branch 'main' as it could not be confidently detected").

3) Do not propose multiple workflow files. Do not add additional jobs or unnecessary comments in the YAML itself.
